...
2. Start the MCP SuperAssistant Proxy server using one of the following commands:
   ...
   This project uses port **3010** (default proxy port 3006 may be in use). Add `--port 3010`:

   ```bash
   npx -y @srbhptl39/mcp-superassistant-proxy@latest --config ./config.json --outputTransport sse --port 3010
   ```
   or 
   ```bash
   npx -y @srbhptl39/mcp-superassistant-proxy --config ./config.json --outputTransport streamableHttp --port 3010
   ```
   or
   ```bash
   npx -y @srbhptl39/mcp-superassistant-proxy --config ./config.json --outputTransport ws --port 3010
   ```
...
4. Enter the local server URL (default: `http://localhost:3010/sse`)
   URL format depends on the --outputTransport method used:
   - For SSE: `http://localhost:3010/sse`
   - For Streamable HTTP: `http://localhost:3010/mcp`
   - For WebSocket: `ws://localhost:3010/message`
