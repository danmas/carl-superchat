...
export class SSEPlugin implements ITransportPlugin {
  ...
  private async createConnection(uri: string): Promise<Transport> {
    try {
      ...
      if (errorMessage.includes('Invalid content type') || errorMessage.includes('text/event-stream')) {
        enhancedError =
          'Not an SSE endpoint. Start the MCP proxy with: npx -y @srbhptl39/mcp-superassistant-proxy@latest --config ./config.json --outputTransport sse --port 3010 — then use URL http://localhost:3010/sse (with /sse).';
